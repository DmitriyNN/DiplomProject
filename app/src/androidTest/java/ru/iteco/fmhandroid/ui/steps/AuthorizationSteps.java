...
public class AuthorizationSteps {
    ...
    public void validAuthorization(){
        Allure.step("Заполнение поля 'login'");
    ...
    public void emptyLoginAndPassword(){
        Allure.step("Оставить  поле 'login' не заполненным");
        auth.login.check(matches(isEnabled()));
        auth.login.perform(replaceText(EMPTYLOGIN));
        Allure.step("Заполнение поля 'password' валидными данными");
        auth.password.check(matches(isEnabled()));
    ...
    public void WrongLogin(){
        Allure.step("Заполнение поля 'login' невалидными данными");
        auth.login.check(matches(isEnabled()));
    ...
    public void WrongPassword(){
        Allure.step("Заполнение поля 'login' валидными данными");
        auth.login.check(matches(isEnabled()));
        auth.login.perform(replaceText(VALIDLOGIN));
        Allure.step("Заполнение поля 'password' невалидными данными");
        auth.password.check(matches(isEnabled()));
